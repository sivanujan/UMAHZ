...
const QUEUE_STYLES = {
    ...
    'Not Arrived': 'bg-stone-100 text-stone-500 border-stone-200',
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-white rounded-xl border border-stone-200/80 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-stone-100 flex items-center space-x-2 bg-stone-50/50">
                        ...
                        <h2 className="font-semibold text-stone-800 text-sm">Check-In Queue</h2>
                    </div>
                    <div className="divide-y divide-stone-100">
                        {checkInQueue && checkInQueue.length > 0 ? checkInQueue.map((c, i) => (
                            <div key={i} className="p-4 flex items-center justify-between">
                                <div>
                                    <p className="text-sm font-medium text-stone-900">{c.client_name}</p>
                                    <p className="text-xs text-stone-500">{c.time}</p>
                                </div>
                            ...
                            <div className="p-6 text-center text-stone-400 text-xs">No one in queue.</div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-stone-200/80 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-stone-100 flex items-center space-x-2 bg-stone-50/50">
                        ...
                        <h2 className="font-semibold text-stone-800 text-sm">Today's Schedule</h2>
                    </div>
                    <div className="divide-y divide-stone-100">
                        {todaysSchedule && todaysSchedule.length > 0 ? todaysSchedule.map((s, i) => (
                            <div key={i} className="p-4">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <p className="text-sm font-medium text-stone-900">{s.client_name}</p>
                                </div>
                                <p className="text-xs text-stone-500 mt-1">{s.practitioner} · {s.room}</p>
                                <p className="text-xs text-stone-400 mt-0.5">{s.time}</p>
                            </div>
                        )) : (
                            <div className="p-6 text-center text-stone-400 text-xs">Nothing scheduled today.</div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-stone-200/80 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-stone-100 flex items-center space-x-2 bg-stone-50/50">
                        ...
                        <h2 className="font-semibold text-stone-800 text-sm">Balances Due</h2>
                    </div>
                    <div className="divide-y divide-stone-100">
                        {balancesDue && balancesDue.length > 0 ? balancesDue.map((b, i) => (
                            <div key={i} className="p-4 flex items-center justify-between">
                                <p className="text-sm text-stone-700">{b.client_name}</p>
                                <p className="text-sm font-bold text-stone-900">{b.amount}</p>
                            </div>
                        )) : (
                            <div className="p-6 text-center text-stone-400 text-xs">No balances due.</div>
                        )}
                    </div>
                </div>
            </div>
