    ...
    return (
        <div className="min-h-screen bg-[#F9F5FB] font-sans antialiased text-slate-800">
            <header className="bg-white border-b border-purple-100">
                <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
                    ...
                    <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-500">
                        {NAV.map((item) => (
                            <Link key={item.name} href={item.href} className="hover:text-[#5B2EFF] transition-colors flex items-center gap-1.5">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-2 text-sm font-semibold text-[#1E0B3C]"
                        >
                            <span className="w-8 h-8 rounded-full bg-purple-100 text-[#5B2EFF] flex items-center justify-center text-xs font-bold">
                        ...
                        {userDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-purple-100 py-1 z-50">
                                <div className="px-4 py-2 text-xs text-slate-500 border-b border-purple-50">
                                    Signed in as <span className="font-semibold text-[#1E0B3C]">{user?.name}</span>
                                </div>
                                <Link
                                    href="/logout"
                                    method="post"
            ...
            <main className="max-w-6xl mx-auto px-6 py-10">
                {title && <h1 className="text-2xl font-bold text-[#1E0B3C] mb-6">{title}</h1>}
                {children}
            </main>
        </div>
    );
